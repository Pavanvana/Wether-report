...
class Home extends Component{
    state = {
        ...
        cityname: "",
        newCityInput: "",
        favoriteCitysList: favoriteCitysList,
        myLocation: ""
    }
    ...
    onClickSearchButton = () => {
        ...
        this.setState({location: searchInput, searchInput: ""}, this.getWetherReport)
    ...
    onClickButton = () => {
        ...
        this.setState(prevState => ({favoriteCitysList: [...prevState.favoriteCitysList, newCity], newCityInput: ""}) )
    }

    onClickLocationButton = () => {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(async position => {
                this.setState({myLocation: position.coords.latitude + "," + position.coords.longitude })
    ...
    render(){
        const {searchInput,weatherImg,temp,windSpeed,humidity, cityname, favoriteCitysList,newCityInput, myLocation} = this.state
        return(
            <div className='app-container'>
                <div className='wether-report-container'>
                    <h1 className='heading'>Wether Report</h1>
                    <div className='search-container'>
                        <input className='search-input' type="search" placeholder='Search...' onChange={this.onChangeCity} value={searchInput}/>
                        <button className='icon-button' onClick={this.onClickSearchButton}><HiOutlineSearch /></button>
                        <button className='icon-button' onClick={this.onClickLocationButton}><HiOutlineLocationMarker/></button>
                    ...
                    <div className='wether'>
                        ...
                        <div className='details'>
                            <div className='each-details'>
                                <img className='humidity-img' src="https://res.cloudinary.com/daflxmokq/image/upload/v1683970998/humidity_tgjwbf.png" alt="humidity"/>
                                <p className='humidity-num'>{humidity}%</p>
                                <p className='humidity-num'>Humidity</p>
                            </div>
                            <div className='each-details'>
                                <img className='wind-speed-img' src="https://res.cloudinary.com/daflxmokq/image/upload/v1683971067/wind_zvfown.png" alt="wind speed"/>
                    ...
                    <div>
                        <div className='favorite-place-heading-container'>
                            ...
                            <div className='new-city-input-container'>
                                <input onChange={this.addNewCityInput} className='type-newcity-input' placeholder='Type new city...' type="text" value={newCityInput}/>
                                <button onClick={this.onClickButton} type="button" className='add-city-button'>Add</button>
                            </div>
                        </div>
                        <ul className='list-container'>
                        {favoriteCitysList.map(each => (
